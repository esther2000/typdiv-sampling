...
def create_arg_parser():
    ...
    parser.add_argument("-g", "--gb_folder", type=str,
...
def binarize(gb_df, mv_feats):
    ...
    for feat in mv_feats.keys():
        for val in range(0, mv_feats[feat]):
            # binarize multi-value features
            gb_df[f"{feat}_{str(val)}"] = (gb_df[f"{feat}"] == str(val)).astype(int).astype(str)
...
def main():
    ...
    mv_feats = {'GB024': 3, 'GB025': 3, 'GB065': 3, 'GB130': 3, 'GB193': 4, 'GB203': 4}  # IDs and num possible values
